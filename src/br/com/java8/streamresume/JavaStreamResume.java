...
public class JavaStreamResume {
  public static void main(String[] args) {
    ...
    // Use Map to stream
    List<Integer> myMappedList =
        Stream.of(1, 2, 3, 4).map(value -> value + 2).collect(Collectors.toList());
    ...
    Optional<Integer> sum =
        Stream.of(1, 3, 5).reduce((accumulative, currentIndex) -> accumulative + currentIndex);

    // Reduce using validation
    String color = "blue";
    List<Car> carList = Arrays.asList(new Car("red", 4), new Car("blue", 4));

    Optional<Car> checkDuplicated =
        carList.stream()
    ...
    boolean anyMatch = Stream.of(1, 2, 3, 4).anyMatch(value -> value == 2);
    ...
    Stream.of(1, 3, 4).peek(value -> System.out.println(value));
    ...
    List<String> vehiclesString =
        cars.filter(car -> car.getWheelsNumber() > 2)
